...
# helpers
# ------------------------------------------------------------------
function bounded_sigmoid(lo::Real, hi::Real)
    lo32 = Float32(lo)
    hi32 = Float32(hi)
    hi32 > lo32 || error("bounded_sigmoid requires hi > lo.")
    return x -> lo32 .+ (hi32 - lo32) .* sigmoid.(x)
end
...
#   row 3: rho2(s)
#   row 4: delta(s)
#   row 5: nu, repeated over nodes
# ------------------------------------------------------------------
struct NodewiseSGCN{P, HS, H1, H2, H3, HN}
    propagation::P
    head_sigma::HS
    head_rho1::H1
    head_rho2::H2
...
function (m::NodewiseSGCN)(g::GNNGraph)
    ...
    Hsummary = dropdims(sum(H, dims = 2), dims = 2)   # hidden × n

    sigma_raw = m.head_sigma(Hsummary)                # 1 × n
    ...
    # graph-level / domain-level summary for Matérn ν
    Hglobal = mean(Hsummary, dims = 2)                 # hidden × 1
    nu_hat  = m.head_nu(Hglobal)                      # 1 × 1

    return site_hat, nu_hat
end
...
function make_nodewise_sgcn(
    ...
    head_sigma = Chain(
        ...
        Dense(head_hidden2,  1, bounded_sigmoid(sigma_range[1], sigma_range[2])),
    )
    ...
    # graph-level head for Matérn smoothness nu
    head_nu = Chain(
        Dense(head_in,       head_hidden1, activation),
        Dense(head_hidden1,  head_hidden2, activation),
        Dense(head_hidden2,  1, bounded_sigmoid(nu_range[1], nu_range[2])),
    )
